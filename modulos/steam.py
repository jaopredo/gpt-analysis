...
def coletar_informacoes_do_jogo(jogo):
    ...
    soup = BeautifulSoup(page.text,"lxml")
    fatia = soup.find("div", class_="details_block") # pega algumas informacoes
    try:
        print(fatia)
        genero = re.search(r'Genre:(.+)\n',fatia.text).group(1).strip() # pega o genero do jogo
    except:
        ...
        for linha in arquivo: #cada linha está nessa forma : Epic Games||||Fortnite||||Gears of War||||Infinity Blade||||Fortnite Chapter 2 Season 8||||Tony Hawk's Pro Skater 1+2
            ...
            for jogo in lista[1:]:
                ...
                try:
                    ...
                    print(error)
steam()
#coletar_informacoes_do_jogo("Grand Theft Auto: The Trilogy - The Definitive Edition steam")
